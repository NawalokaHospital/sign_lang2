...
def text_mode(cam):
    ...
    while True:
        ...
        if len(contours) > 0:
            ...
            word = ""
        blackboard = np.zeros((480, 640, 3), dtype=np.uint8)
        cv2.putText(blackboard, "Hand Sign Detection App", (180, 50), cv2.FONT_HERSHEY_TRIPLEX, 1.0, (255, 0, 0))
        print(text)
        ...
        print("Full sentences --" + word)
        if(word != ""):
            is_write = True
            write_word = word
        if((word == "") & is_write):
            is_write = False
            file = open("sign_value.txt", "a", encoding="utf-8")
